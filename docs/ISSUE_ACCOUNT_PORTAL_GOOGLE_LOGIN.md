...
Browser
  -> walnut-website /login
    -> Google OAuth
      -> walnut-website /api/auth/google/callback
        -> website server validates Google identity
...
```text
https://www.walnut.xxx/api/auth/google/callback
```

如果后续拆子域名，推荐：

```text
https://accounts.walnut.xxx/api/auth/google/callback
```
...
app/
  ...
  api/auth/google/start/route.ts
...
```text
lib/account/
  auth-session.ts                # httpOnly cookie/session abstraction
  csrf.ts                        # CSRF token strategy
  google-oauth.ts                # provider adapter
...
目标：网页登录后拥有 website 自己的 session，而不是把 Google token 暴露给前端。

要求：

- 使用 httpOnly、Secure、SameSite=Lax/Strict cookie。
- Session 中只保存 website session id 或加密的最小 claims。
...
- OIDC nonce。
- state 绑定 CSRF 与 return intent。
- Callback 中验证 ID token audience、issuer、nonce、email_verified。
...
| Route | `/api/auth/google/start`、`/api/auth/google/callback`、logout |
